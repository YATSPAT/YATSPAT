    ...
    <div className="shrink-0 rounded-none border border-pink-400/20 bg-surface-800/60 backdrop-blur p-3 w-full">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[10px] font-mono text-slate-500">
          every {fmtInterval(p.intervalMinutes)}
        </span>
      </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        {p.rules.map((r, i) => (
          <span
            key={i}
            className="px-1.5 py-0.5 rounded-none bg-surface-900/70 border border-slate-700/40 text-slate-300 text-[10px]"
          >
            {ACTION_ICON[r.type] ?? "•"}{" "}
            {r.type === "distribute" ? "ATA growth" : r.type} {r.pct}%
          </span>
        ))}
      ...
      {err && p.lastRunSummary && (
        <div
          className="mt-2 px-2 py-1 rounded-none bg-rose-500/10 border border-rose-500/20 text-[10px] text-rose-300 font-mono truncate"
          title={p.lastRunSummary}
        >
          {p.lastRunSummary.length > 60
            ? p.lastRunSummary.slice(0, 60) + "…"
            : p.lastRunSummary}
        </div>
      )}
    </div>
  ...
  return (
    <div className="glass-card p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-pink-300 tracking-wide">
          Live Pipes
        </h3>
        <span className="flex items-center gap-1.5 text-[10px] font-mono text-emerald-300">
          <span
            aria-hidden="true"
            className="w-1.5 h-1.5 rounded-none bg-emerald-400 animate-pulse"
          />
      ...
      {!loaded ? (
        ...
        <div className={horizontal ? "py-5 text-center" : "py-8 text-center"}>
          ...
          <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
            {horizontal ? (
              "Yours could be the first — build one above."
            ) : (
              <>
                Yours could be the first —<br />
                build one on the right.
              </>
            )}
          </p>
        </div>
